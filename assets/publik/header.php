<body>
<header class="header" data-header>
    <div class="container">

      <h1>
        <a href="#" class="logo"><?php echo $title ?></a>
      </h1>

      <button class="menu-toggle-btn icon-box" data-menu-toggle-btn aria-label="Toggle Menu">
        <span class="material-symbols-rounded  icon">menu</span>
      </button>

      <nav class="navbar">
        <div class="container">

          <ul class="navbar-list">

            <li>
              <a href="/monitoring/admin/login.php" class="navbar-link active icon-box">
                <span class="material-symbols-rounded  icon">grid_view</span>

                <span>Home</span>
              </a>
            </li>
  
            <li>
              <a href="login.php" class="navbar-link icon-box">
                <span class="material-symbols-rounded  icon">settings</span>

                <span>Pemeliharaan</span>
              </a>
            </li>
            <div id="datetime" style="font-size: 16px; font-family: 'Courier New', Courier, monospace; position: fixed; top: 10px; right: 10px;">
  <i class="far fa-clock"></i>
</div>
</ul>

        </div>
      </nav>

    </div>
  </header>


  